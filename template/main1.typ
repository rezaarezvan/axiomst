...
// Example of a problem with a margin note
...
#margin-note[
  Remember that a vector space must satisfy all eight axioms of vector spaces. This proof relies on the properties of addition and scalar multiplication.
]

// Solution with theorem environment
...
// Example of using multiple columns for comparison
#pagebreak()
...
// Example of a theorem with multiple corollaries
#pagebreak()
...
#proof[
  Let $K = \text{ker}(T)$ and let $\{v_1, v_2, \ldots, v_k\}$ be a basis for $K$.

  Extend this to a basis $\{v_1, \ldots, v_k, v_{k+1}, \ldots, v_n\}$ for $V$.

  We claim that $\{T(v_{k+1}), \ldots, T(v_n)\}$ is a basis for $\text{im}(T)$.
  ...
  By the linear independence of the basis of $V$, all coefficients must be zero. So $\{T(v_{k+1}), \ldots, T(v_n)\}$ is linearly independent.

  For spanning, any $w \in \text{im}(T)$ can be written as $w = T(v)$ for some $v \in V$. We can write $v = \sum_{i=1}^n c_i v_i$. Since $T(v_1) = \ldots = T(v_k) = 0$, we have $w = \sum_{i=k+1}^n c_i T(v_i)$.

  Thus, $\dim(\text{im}(T)) = n - k = \dim(V) - \dim(\text{ker}(T))$.
]

#columns(count: 2)[
  #corollary(title: "Injective Case")[
    If $T$ is injective, then $\text{ker}(T) = \{0\}$, so $\dim(\text{im}(T)) = \dim(V)$.

    This means $T$ preserves dimension.
  ]
][
  #corollary(title: "Surjective Case")[
    If $T$ is surjective, then $\text{im}(T) = W$, so $\dim(\text{ker}(T)) = \dim(V) - \dim(W)$.
...
#margin-note(
  [The Rank-Nullity Theorem is a fundamental result in linear algebra that connects the dimensions of the kernel and image of a linear transformation.

  This result forms the basis for understanding many properties of linear maps between vector spaces.],
  side: "left",
  color: red.darken(20%)
)
